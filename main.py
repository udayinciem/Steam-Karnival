...
# db = client["file_tracking_db"]
# followup_collection = db["followups"]
# # Insert something into the collection
# followup_collection.insert_one({"message": "Hello Mongo!"})
...
async def whatsapp_webhook(From: str = Form(...), Body: str = Form(...), MediaUrl0: str = Form(None)):
    ...
    try:
        # Step 2: Update cache if needed and search
        # Force cache refresh on every query to ensure latest data
        print("Forcing cache refresh to ensure latest data")
        program_cache['combined_data'] = None  # Force cache refresh
        program_cache['excel_data'] = None  # Force Excel reload
        
        # Get combined data from cache
        print("Updating cache if needed...")
        combined_data = update_cache_if_needed()
        
        # Search using cached data
        print("Searching through data...")
        response = search_program_data(extracted, user_message, combined_data)
    except Exception as e:
        print(f"Error processing request: {e}")
        response = ("I'm having trouble accessing the program data right now. "
                   "Please try again in a moment. If you just uploaded a PDF, "
                   "it might take a few seconds to process.")

    # Step 3: Generate a natural, human-like WhatsApp reply
    final_reply = generate_human_like_reply(user_message, response)

    # Step 4: Send the message via Twilio
    send_result = send_whatsapp_message(user_number, final_reply)
    
    # Handle send errors
    if isinstance(send_result, dict) and send_result.get("status") == "error":
        error_type = send_result.get("type")
        if error_type == "rate_limit":
            ...
            return {
                "status": "error",
                "error": "twilio_error",
                "message": send_result.get("message", "Error sending message")
            }
...
def search_program_data(extracted, user_message, combined_data=None):
    ...
    if is_schedule_question and combined_data:
        ...
        # Search combined_data for exact matches first
        program_name_terms = [w for w in user_message.split() if len(w) > 3]
        matching_programs = []
        
        for program in combined_data:
            program_name = program.get("Program Name", "").upper()
            stage = program.get("Stage", "")
            category = program.get("Category", "")
            date = program.get("Date", "")
            
            # Check if any program name terms match
            if any(term.upper() in program_name for term in program_name_terms):
                # Prefer Excel data (Source == "excel") for schedule questions
                if program.get("Source") == "excel":
                    matching_programs.append(program)
                elif not any(p.get("Source") == "excel" for p in matching_programs):
                    # Only add PDF programs if no Excel match found
                    matching_programs.append(program)
        
        if matching_programs:
            print(f"Found {len(matching_programs)} matching program(s) in Excel data")
...
def generate_human_like_reply(user_message, info_text):
    """
    Use OpenAI to generate a polite, conversational response.
    """
    try:
        response = client.chat.completions.create(
            model="gpt-3.5-turbo",
            ...
            messages=[
                ...
                - Maintain the exact formatting from the input (e.g., *bold* text, stage groupings)
                - CRITICAL: NEVER truncate or summarize program lists - include ALL items provided in the input
                - CRITICAL: Preserve ALL stage groupings and program details exactly as provided
                - Never say "and many more" or "etc."; list exact items only
                ...
                {"role": "user", "content": user_message},
                {"role": "assistant", "content": f"{info_text}"},
                {"role": "user", "content": "Rewrite the above so it reads naturally and concisely for WhatsApp. Do NOT add emojis. Do not remove or shorten any list items or details that appear above."}
            ]
        )
        text = response.choices[0].message.content.strip()
        # Fallback sanitizer to remove accidental emojis
        try:
